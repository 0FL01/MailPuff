...
Tech stack target: Rust `2024` (`rustc >= 1.94`), Docker-only runtime, `tokio`, `tracing`, typed env config, in-memory state only. Historical Go code is preserved on branch `go`; the primary branch is Rust-only.
...
- Never bypass `src/viewer` sanitization when storing or serving email HTML.
...
### IMAP Polling
- Polling should route through `src/mail_source::MailSource`, not directly through IMAP-specific state.
...
- If HTML is missing, `text/plain` is escaped and wrapped in `<pre>`; messages with no usable body are skipped by orchestration.
