...
import (
    ...
    "mail-to-tg/pkg/config"
    "mail-to-tg/pkg/email"
    imapPkg "mail-to-tg/pkg/imap"
    "mail-to-tg/pkg/telegram"
    "mail-to-tg/pkg/viewer"
)
...
func main() {
	cfg := config.Load()
	log.Printf("starting mail-to-tg with poll interval %s, mailbox %s", cfg.PollInterval, cfg.Mailbox)
	...
	for {
		...
		func() {
            ...
            for uid, em := range emailsMap {
                ...
                processed[uid] = struct{}{}
                // Удаление теперь выполняется колбэком store (по TTL/лимиту просмотров)

				// Отметка прочитанным теперь переносится на событие первого открытия HTML-страницы
			}
		}()
		time.Sleep(cfg.PollInterval)
	}
